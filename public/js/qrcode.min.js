 ...
 * Generates high quality QR Codes in canvas/svg with zero dependencies.
 */
var QRCode;
(function(){
...
function QRCodeModel(typeNumber,errorCorrectLevel){this.typeNumber=typeNumber;this.errorCorrectLevel=errorCorrectLevel;this.modules=null;this.moduleCount=0;this.dataCache=null;this.dataList=new Array()}
...
QRRSBlock.RS_BLOCK_TABLE=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],[1,44,16],[1,70,55],[1,70,44],[2,35,17],[2,35,13],[1,100,80],[2,50,32],[2,50,24],[4,25,9],[1,134,108],[2,67,43],[2,33,15,2,34,16],[2,33,11,4,34,12]];
QRRSBlock.getRSBlocks=function(typeNumber,errorCorrectLevel){var rsBlock=QRRSBlock.getRsBlockTable(typeNumber,errorCorrectLevel);if(rsBlock==undefined)throw new Error("bad rs block @ typeNumber:"+typeNumber+"/errorCorrectLevel:"+errorCorrectLevel);var length=rsBlock.length/2;var list=new Array();for(var i=0;i<length;i++){var count=rsBlock[i*2+0];var totalCount=rsBlock[i*2+1];for(var j=0;j<count;j++)list.push(new QRRSBlock(totalCount,totalCount-QRRSBlock.RS_BLOCK_TABLE[(typeNumber-1)*4+errorCorrectLevel][1]))}return list};
QRRSBlock.getRsBlockTable=function(typeNumber,errorCorrectLevel){switch(errorCorrectLevel){case QRErrorCorrectLevel.L:return[[1,26],[1,44],[1,70],[1,100],[1,134]][typeNumber-1]||[1,134];case QRErrorCorrectLevel.M:return[[1,26],[1,44],[1,70],[2,50],[2,67]][typeNumber-1]||[2,67];case QRErrorCorrectLevel.Q:return[[1,26],[1,44],[2,35],[2,50],[2,33,2,34]][typeNumber-1]||[2,33,2,34];case QRErrorCorrectLevel.H:return[[1,26],[1,44],[2,35],[4,25],[2,33,4,34]][typeNumber-1]||[2,33,4,34]}};

function QRBitBuffer(){this.buffer=new Array();this.length=0}
QRBitBuffer.prototype={get:function(i){var grp=Math.floor(i/8);return((this.buffer[grp]>>>(7-i%8))&1)==1},put:function(num,length){for(var i=0;i<length;i++)this.putBit(((num>>>(length-i-1))&1)==1)},getLengthInBits:function(){return this.length},putBit:function(bit){var grp=Math.floor(this.length/8);if(this.buffer.length<=grp)this.buffer.push(0);if(bit)this.buffer[grp]|=(0x80>>>(this.length%8));this.length++}};
...
QRCode.prototype.makeCode=function(text){
  ...
  var qrcode=new QRCodeModel(0,this.options.correctLevel);
  qrcode.addData(text);
  qrcode.make();
